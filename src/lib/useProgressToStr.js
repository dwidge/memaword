...
export const calcStrProgress = (list) => {
  const startSeconds = list.map(([level, next]) => +next).reduce(least, Infinity);
const relativeList=list.map(([level, next]) => [level, daysSeconds(next - startSeconds),(next - startSeconds)%secondsPerDay])
...
export const useSimpleProgressStr = ([str, setStr]) => {
  ...
  const setProgress = (list) => 
    setStr(calcStrProgress(list));
